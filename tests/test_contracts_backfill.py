...
def test_all_agents_contracts(agent_class):
    ...
    assert hasattr(output.metadata, "__dict__") or isinstance(
    ...
    metadata_dict = (
        output.metadata if isinstance(output.metadata, dict) else output.metadata.__dict__
    )
    assert (
        "agent_name" in metadata_dict or "agent" in metadata_dict or "source" in metadata_dict
...
def test_all_advisors_contracts(advisor_class):
    ...
    dummy_output = AgentOutput(
        content="test content", artifacts=[], metadata={"agent": "TestAgent"}
    )

    # Test review returns AdvisorReview
    review = advisor.review(output=dummy_output, task="test task", context={})
    ...
    # Validate types
    assert isinstance(
        review["score"], (int, float)
    ), f"{advisor_class.__name__} score must be numeric"
    ...
    assert isinstance(
        review["critical_issues"], list
    ), f"{advisor_class.__name__} critical_issues must be list"
    assert isinstance(
        review["suggestions"], list
    ), f"{advisor_class.__name__} suggestions must be list"
    assert isinstance(review["summary"], str), f"{advisor_class.__name__} summary must be str"
    assert review["severity"] in {
        "low",
        "medium",
        "high",
        "critical",
