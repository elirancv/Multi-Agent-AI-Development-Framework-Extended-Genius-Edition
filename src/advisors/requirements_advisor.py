...
class RequirementsAdvisor(BaseAdvisor):
    ...
    def review(self, output: AgentOutput, task: str, context: Dict[str, Any]) -> AdvisorReview:
        ...
        if len(content) < 300:
            suggestions.append("Expand functional requirements to at least 5 concrete bullets.")

        severity: Literal[low, medium, high, critical] = "medium" if issues else "low"
        score = max(0.0, 1.0 - 0.15 * len(issues))
        approved = (
            score >= 0.80 and "acceptance criteria" in content and "non-functional" in content
        )
