...
from src.agents.code_skeleton_marketing import CodeSkeletonMarketingAgent
...
from src.agents.screenshot_agent import ScreenshotAgent
from src.agents.static_linter_agent import StaticLinterAgent
from src.agents.zip_packager_agent import ZipPackagerAgent
...
CORE_AGENTS: Dict[str, Type[BaseFunctionalAgent]] = {
    ...
    "ZipPackagerAgent": ZipPackagerAgent,
    "ScreenshotAgent": ScreenshotAgent,
    "CodeSkeletonMarketingAgent": CodeSkeletonMarketingAgent,
...
def advisor_factory(name: str) -> BaseAdvisor:
    ...
    if name not in advisors:
        ...
        raise KeyError(f"Advisor '{name}' not found. Available advisors: {available}")
